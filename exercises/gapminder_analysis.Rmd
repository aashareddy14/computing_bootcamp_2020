...
```{r ex2_part2, echo = FALSE}
# create plot here
gap_canada <- gap_upd %>%
   filter(country == "Canada")

ggplot(data = gap_canada, aes(x = lifeExp, y = gdpPercap)) +
...
ggplot(data = gap_africa, aes(x = lifeExp, y = gdpPercap)) +
...
africa_eu_1992 <- gap_upd %>%
   filter(continent %in% c("Africa", "Europe"), year == 1992)

ggplot(data = africa_eu_1992, aes(x = lifeExp, y = gdpPercap, color = continent)) +
   geom_point()
```
